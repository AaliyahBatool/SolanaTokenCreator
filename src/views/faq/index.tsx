      ...
      <div className='container'>
        <div className='mb-10 flex items-end justify-between'>
          <div className='max-auto max-w-2xl text-center'>
            ...
            <p className='text-default-200 text-sm font-medium'>Lorem ipsum dolor sit amet consectetur adipisicing elit.</p>
          </div>
        </div>
          ...
          <div className='hs-accordion-group space-y-4'>
            {
              questions.map((question, index) => (
                <div key={index} className={`hs-accordion bg-default-950/40 overflow-hidden rounded-lg border border-white/10 backdrop-blur-3xl`} id={question.id}>
                  <button className='hs-accordion-toggle inline-flex items-center justify-center justify-between gap-x-3 px-6 py-4 text-left capitalize text-white transition-all' aria-controls={`faq-accordion-${index + 1}`}>
                  ...
                  <div id={`faq-accordion-${index + 1}`} className='hs-accordion-content w-full overflow-hidden transition-[height] duration-300' aria-labelledy={question.id}>
